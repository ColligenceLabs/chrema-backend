const {HistoryModel, AdminModel, UserModel, SerialModel} = require('../models');
const {addMongooseParam} = require('../utils/helper');

module.exports = {
    findByTxId: async function (id) {
        let tx = await HistoryModel.findOne({_id: id})
            .populate('serial_id')
    ...
    findAllTx: async function (findParams, pagination = null) {
        try {
            if (pagination) {
                var txs = await HistoryModel.find(findParams)
    ...
    findOneTx: async function (where) {
        let tx = await HistoryModel.findOne(where);
    ...
    count: async function (where) {
        let count = await HistoryModel.countDocuments(where);
        if (!count) {
            return null;
        }
        return count;
    ...
    createTx: async function (newTx) {
        let tx = await HistoryModel.create(newTx);
        if (!tx) {
            return null;
        }
        return tx;
    ...
    update: async function (id, where) {
        try {
            let tx = await HistoryModel.updateOne({_id: id}, {$set: where});
            return tx;
        } catch (error) {
            return error;
        }
    ...
    getTxByseller: async function (seller) {
        try {
            let txs = await HistoryModel.find({seller: seller})
                .sort({createdAt: -1, _id: 1})
                .populate('serial_id')
                .populate({path: 'seller', select: 'admin_address'})
                .populate({path: 'buyer', select: 'uid'});
