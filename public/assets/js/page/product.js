const View = {
    ...
    table: {
        ...
        __generateDTRow(data, k) {
            ...
            return [
                ...
                data.collection_id == null ?  '-' : (data.collection_id.name == null ? '-' : data.collection_id.name),
                (data.selling == true ? `<span class="badge badge-pill badge-green m-b-5">On sale</span>` : `<span class="badge badge-pill badge-light m-b-5">Off sale</span>`) +
                `<span class="badge badge-pill badge-magenta m-b-5">Total Mint: ${data.quantity}</span>
                <span class="badge badge-pill badge-geekblue m-b-5">Selling Quantity: ${data.quantity_selling}</span>
                <span class="badge badge-pill badge-purple m-b-5">Prices: ${data.price}</span>`,
                data.company_id == null ?  '-' : (data.company_id.name == null ? '-' : data.company_id.name),
    ...
    modals: {
        CreateProduct: {
            ...
            setVal(data){
                $(`${this.resource}`).find('.modal-title').html(`NFT Mint`);
                    ...
                    <div class="row">
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control data-price" id="price" min="0" step="0.001">
                            </div>
                        </div>
                    </div>
                `);
        ...
        UpdateProduct: {
            ...
            setVal(data){
                $(`${this.resource}`).find('.modal-title').html(`<i class="anticon anticon-info-circle m-r-5"></i> NFT detail`);
        ...
        UpdateStatus: {
            ...
            setDefaul(){ },
            setVal(){
                $(`${this.resource}`).find('.modal-title').html(`Update NFT status`);
            },
            getVal(){
                return {
        ...
        DeleteOneUser: {
            ...
            setDefaul(data){
                $(`${this.resource}`).find('.btn.btn-default').remove();
                if (data) {
                    $(`${this.resource}`).find('.modal-footer').prepend(`<button type="button" data-dismiss="modal" class="btn btn-default none">Cancel</button>`);
                }
            },
            setVal(title){
                $(`${this.resource}`).find('.modal-title').html(title);
            },
            getVal(){
                return {
        ...
        UpdateTimeProduct: {
            ...
            setDefaul(){ },
            setVal(data){
                $(`${this.resource}`).find('.modal-title').html(`Update time ${data} NFTs`);
            },
            getVal(){
                var id_array = [];
                View.table.listSelectedRows().map(v => id_array.push(v._id))
...
(() => {
    ...
    View.selectedAction.onAtrAction("Sale Schedule", (item) => {
    ...
    View.selectedAction.onAtrAction("Delete", (item) => {
        var hasPush = true;
        var title = "";
        Object.entries(View.table.__selected).map(v => {
            if (v[1].collection_id != null) hasPush = false;
        })
        View.modals.DeleteUser.setDefaul(hasPush)
        if (hasPush){
            title = `Delete ${Object.entries(View.table.__selected).length} NFTs`;
            View.modals.DeleteUser.setVal(title);
            ...
            title = "Cannot remove NFTs belonging to the collection."
            View.modals.DeleteUser.setVal(title);
            View.modals.DeleteUser.show();
            View.modals.DeleteUser.unbindAll();
    ...
    View.table.onCreate("Create", () => {
        ...
        Api.Category.Get()
            .done(res => {
                View.modals.CreateProduct.setCategory(res.data)
            })
            .fail(err => {
                View.helper.showToastError('Error', 'Something Wrong'); 
            })
        ...
        Api.Company.GetAll(View.table.pagination.page, View.table.pagination.pageSize, filter)
            .done(res => {
                ...
                View.modals.CreateProduct.onPush("Save", (fd) => {
                    View.helper.showToastProcessing('Processing', 'Create NFT !');
    ...
    View.table.onRowUpdateStatus("Update Status", (item) => {
        ...
        View.modals.UpdateStatus.onPush("Save", () => {
            ...
            Api.Product.UpdateStatus(id, data)
                .done(res => {
                    View.helper.showToastSuccess('Success', 'Update successful'); 
                    View.table.updateColumn(id, status, 9)
                })
                .fail(err => {
                    View.helper.showToastError('Error', 'Something Wrong'); 
                })
            ...
            .done(res => {
                ...
                View.modals.UpdateProduct.onPush("Save", () => {
                    ...
                    Api.Product.UpdateStatus(id, View.modals.UpdateProduct.getVal())
                        .done(res => {
                            View.helper.showToastSuccess('Success', 'Update successful'); 
                            View.table.updateColumn(id, status, 44)
                        })
                        .fail(err => {
                            View.helper.showToastError('Error', 'Something Wrong'); 
                        })
    ...
    View.table.onRowAction("Delete", (id) => {
        Api.Product.GetOne(id).done(res => {
            if (res.data.collection_id == null) {
                View.modals.DeleteUser.setDefaul(true)
                View.modals.DeleteOneUser.show();
                View.modals.DeleteOneUser.setVal(`Are you sure delete NFT?`);
                View.modals.DeleteOneUser.unbindAll();
                View.modals.DeleteOneUser.onPush("Save", () => {
                    View.helper.showToastProcessing('Processing', 'Delete NFT !');
                ...
                View.modals.DeleteUser.setDefaul(false)
                View.modals.DeleteOneUser.show();
                View.modals.DeleteOneUser.setVal('Cannot remove NFTs belonging to the collection.');
                View.modals.DeleteOneUser.unbindAll();
                View.modals.DeleteOneUser.onPush("Save", () => {
                    View.modals.DeleteOneUser.hide();
                });
    ...
    function LoadData(){
        ...
        Api.Product.GetAll(View.table.pagination.page, View.table.pagination.pageSize, 0, filter)
            .done(res => {
                console.log(res);
                loadOrders(res);
            });
