...
module.exports = {
    ...
    async indexTxs(req, res, next) {
        try {
            ...
            const txs = await txRepository.findAllTx(findParams, {page, perPage});
            if (!txs) {
    ...
    async getDetailTx(req, res, next) {
        try {
            ...
            const tx = await txRepository.findByTxId(req.params.id);
            if (!tx) {
                return handlerError(req, res, ErrorMessage.TX_IS_NOT_FOUND);
            }
    ...
    async updateTx(req, res, next) {
        try {
            ...
            const updateTx = await txRepository.update(req.params.id, data);
            if (!updateTx) {
                return handlerError(req, res, ErrorMessage.UPDATE_TX_IS_NOT_SUCCESS);
            }
    ...
    async deleteTx(req, res, next) {
        try {
            ...
            const deleteTx = await txRepository.update(req.params.id, {
...
async function getFindParams(filters) {
    ...
    if (filters.keyword) {
        const buyerIds = await txRepository.findByBuyerAddress(filters.keyword);
        const sellerIds = await txRepository.findBySellerAddress(filters.keyword);
        const serialIds = await txRepository.findBySerialTokenIds(filters.keyword);
