        ...
        try {
            ...
            for (let i = 0; i < ownserSerials.length; i++) {
                for (let j = 0; j < nfts.length; j++) {
                   if (nfts[j].id == ownserSerials[i].nft_id._id) {
                       console.log("yes");
                       nfts[j].own_serial_id = ownserSerials[i].nft_id._id;
                   }
                }
            }
    ...
    async myNFTs(req, res, next) {
        try {
            ...
            let findParams = {
                buyer: user._id,
                // status: consts.TRANSACTION_STATUS.PROCESSING,
                $or: [{status: consts.TRANSACTION_STATUS.PROCESSING},{status: consts.TRANSACTION_STATUS.ERROR}]
            };
